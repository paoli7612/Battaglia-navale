...
int allow_place(){
	for (int n=0; n<len; n++){
		if (dir && get_pos_mat(nPLAYER_1, HIDE, Xs+n, Ys) != EMPTY){return false;}
		else if (get_pos_mat(nPLAYER_1, HIDE, Xs, Ys+n) != EMPTY){return false;}
	}
	return true;
}
...
void draw_mat(int slot){
	for (int y=0; y<10; y++)
		for (int x=0; x<10; x++){
			...
			if (h == SHOT) color(COLOR_SHOT);		//COLPITO
			if (h == SUNK) color(COLOR_SUNK);		//AFFONDATO
			if (h == NIGH) color(RED);				//ACQUA
			if (h == EMPTY) color(COLOR_EMPTY); 	//nave posizionabile
			if (h == TAKEN) color(COLOR_SHIP); 		//NAVE pulita
			if (d == WATER) color(COLOR_WATER);		//
			if (d == SUNK) color(COLOR_SUNK);
			if (d == SHOT) color(COLOR_SHOT);
...
int pos_ships(){
	...
	for (int n=0; n<SHIPS_MAX; n++){
		...
		while (not placed){
			...
			switch(key){
				// FRECCE
				case K_LEFT:{if (Xs>0)Xs--;break;}
				case K_RIGHT:{if (Xs<9-(len-1)*dir)Xs++; break;}
				case K_UP:{if (Ys>0)Ys--; break;}
				case K_DOWN:{if (Ys<9-(len-1)*cross_dir(dir))Ys++; break;}
				// Q W E R
